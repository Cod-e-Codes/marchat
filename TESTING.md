...
**Note**: Narrative release summaries live in **[CHANGELOG.md](CHANGELOG.md)** (and on GitHub Releases). This is a foundational test suite with good coverage for smaller utility packages and growing coverage for client and server components. **Overall statement coverage is 42.4%** across all packages in the main module, computed from the merged profile at the repo root (for example the `coverage` file or another path passed to `go test -coverprofile=... ./...`). Regenerate summaries with `go tool cover -func=<same-path>`. On **Windows PowerShell**, prefer a profile filename **without** a `.out` suffix (e.g. `mergedcoverage`, `coverage`, or per-package `ph` / `sv`) so the argument is not misparsed. The nested **`plugin/sdk`** module (separate `go.mod`) is not included in that merged number; its package statement coverage is **58.8%** when measured with `cd plugin/sdk && go test -coverprofile=sdkcover ./... && go tool cover -func=sdkcover`.
...
| `plugin/manager/manager_test.go` | Plugin Manager | `validatePluginName`, plugin state fallbacks, download checksum and traversal guards, archive binary execute bit, staged install rollback, installation, uninstallation, command execution |
...
| `plugin/host` | 64.6% | Medium | 721 | Medium |
| `client/config` | 58.0% | Medium | 1993 | Medium |
| `internal/doctor` | 66.5% | Medium | 828 | Medium |
| `plugin/store` | 47.0% | Medium | 552 | Medium |
| `cmd/license` | 42.2% | Medium | 160 | Small |
| `server` | 39.8% | Low | 7495 | Large |
| `plugin/manager` | 63.5% | Medium | 757 | Medium |
| `client/exthook` | 24.1% | Low | 204 | Small |
| `client` | 28.1% | Low | 6372 | Large |
...
**Overall coverage: 42.4%** (all packages in the main module; merged profile `coverage` or another `-coverprofile` path)
...
- **Plugin Manager**: Installation, store download paths, checksum and archive guards, enable/disable with persisted state (63.5%)
...
| `client/render.go` | 82.3% | client | TUI rendering helpers |
...
| `server/hub.go` | 58.1% | server | WebSocket hub management, channels, DMs |
| `client/code_snippet.go` | 53.4% | client | Code snippet TUI component |
| `server/request_context.go` | 54.6% | server | WebSocket Origin checks, trusted-proxy client IP |
...
| `plugin/manager/manager.go` | 63.5% | plugin/manager | Plugin management |
| `server/admin_panel.go` | 15.9% | server | Admin panel functionality |
| `server/client.go` | 14.7% | server | Client management, message type routing |
...
| `client/main.go` | 12.7% | client | Client main application |

### Areas for Future Testing
- **Server Package**: Advanced WebSocket handling, complex message routing scenarios (current: 39.8%)
- **Client Package**: WebSocket communication, full TUI integration (current: 28.1%)
- **Plugin Host**: Broader command/response paths and failure modes beyond the minimal IPC test plugin (current: 64.6%)
- **Plugin Manager**: Store download, checksum, archive extraction, and install edge cases (current: 63.5%)
...
- **Coverage by Package** (statement %, merged profile): 88.1% (`shared`), 87.1% (`plugin/license`), 80.3% (`client/crypto`), 73.2% (`config`), 64.6% (`plugin/host`), 58.0% (`client/config`), 66.5% (`internal/doctor`), 63.5% (`plugin/manager`), 47.0% (`plugin/store`), 42.2% (`cmd/license`), 39.8% (`server`), 29.4% (`client`), 24.1% (`client/exthook`), 13.7% (`cmd/server`)
- **Nested `plugin/sdk` coverage**: **58.8%** statements (run inside `plugin/sdk`; not part of the root merged profile).
- **Overall Coverage**: **42.4%** across main-module packages (regenerate with `go test -coverprofile=mergedcoverage ./...` then `go tool cover -func=mergedcoverage`; on PowerShell use a profile path **without** a `.out` suffix, for example `mergedcoverage`, `ph`, or `sv`, so `-func=` is not misparsed)
- **Lines of code (approx.)**: non-test `.go` lines per package directory, same totals as the **Current Coverage Status** table (e.g. `server` 7495, `client` 6372); re-count with:
