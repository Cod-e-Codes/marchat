...
- Moderation: permanent bans and unexpired temp kicks load from `ban_history` on hub start; in-memory maps are the hot path. See lock ordering comment on `Hub` in `hub.go`.
