...
- **URLs**: `prepareURLWrapping` (non-breaking hyphens in hosts), `markURLsForWrap` / `applyURLMarkers` so hyperlink color and underline survive line breaks without styling continuation indent. Click-to-open (`findURLAtTranscriptClick` in `render.go`) stitches wrapped continuation lines, trims viewport padding, expands partial matches from visible message bodies, and uses `chatPanelOrigin()` (header + box border + banner) for mouse coordinates; only the URL under the cursor opens.
