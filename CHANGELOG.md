...
- **Server**: **Fix:** active permanent bans and unexpired temp kicks load from `ban_history` on hub start (`expires_at` NULL = permanent; non-NULL = kick expiry), so moderation survives process restart. Pre-upgrade open rows without `expires_at` load as permanent.
- **Server**: Database schema uses versioned migrations (`MigrateSchema`, `schema_version` table) and **hard-fails** startup when required tables or columns are missing instead of logging warnings and continuing.
