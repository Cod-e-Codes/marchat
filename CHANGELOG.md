...
- **Plugins**: Plugin store downloads validate SHA-256 checksums before extraction (HTTP and `file://`), reject oversize archives, parse `file://` paths correctly on Linux and Windows, detect archive type from the URL path (including query strings), extract to a staging directory with zip-slip checks, roll back failed updates, and set the execute bit on the plugin binary by exact name match after ZIP/TAR extract.
