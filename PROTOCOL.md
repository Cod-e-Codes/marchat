...
For optional graphical clients that implement this protocol, see [README.md](README.md#optional-graphical-clients). For a standalone Go WebSocket bot SDK (not a plugin; does not import this module), see [marchat-bot](https://github.com/Cod-e-Codes/marchat-bot).
...
- **Text and DM messages on the wire**: Same JSON message shape as plaintext chat; set `encrypted` to `true`. Applies to `type` `text` and `dm`. The reference client uses `dm` for typed DM compose, `:dm`, and code snippets submitted while a DM thread is open. The `content` field is **standard base64** encoding of **nonce ‖ ciphertext** (nonce first, 12 bytes, then the Poly1305-sealed ciphertext). The plaintext decrypted by the AEAD is a JSON object representing the inner chat message (e.g. sender, content, type, timestamp) as produced by the reference client.
...
- On successful handshake:
  - Sends up to **50** recent messages visible to the authenticated user (channel/public rows with empty `recipient`, plus DMs where the user is sender or recipient). The query applies the limit after visibility filtering, not on raw rows. The same full recent-window replay happens on every new WebSocket session after a disconnect, including when the user reconnects with no new traffic since the last session. `user_message_state` records `last_seen` only; it does not gate replay. When **`MARCHAT_BAN_HISTORY_GAPS`** is enabled, messages sent while the user was banned may be omitted after fetch, so replay can be shorter than 50 lines. Clients that keep a local transcript in memory should **replace** or **dedupe** against that replay instead of appending it blindly, or they will show duplicate lines. The reference TUI clears its transcript (messages, reactions, typing state, and cached received-file metadata) when a connection succeeds so the replay is the single source of truth for the visible scrollback window.
