...
- **Text and DM messages on the wire**: Same JSON message shape as plaintext chat; set `encrypted` to `true`. Applies to `type` `text` and `dm`. The `content` field is **standard base64** encoding of **nonce ‖ ciphertext** (nonce first, 12 bytes, then the Poly1305-sealed ciphertext). The plaintext decrypted by the AEAD is a JSON object representing the inner chat message (e.g. sender, content, type, timestamp) as produced by the reference client.
