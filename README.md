...
- **Server / database**: Dialect-aware schema and operations across SQLite, PostgreSQL, and MySQL/MariaDB; durable persistence for reactions and read receipts; message-state layer with retention-friendly cleanup
- **Reliability**: Serialized WebSocket writes to avoid concurrent ping panics; MySQL index DDL and retention `DELETE` compatibility fixes
- **Admin TUI**: Layout, logs view, and heading alignment fixes
- **Doctor**: Reports database dialect and DSN validity; covers empty durable-state tables
- **Docs / protocol**: ARCHITECTURE, PROTOCOL, and multi-DB notes aligned with durable state and validation
- **Tests**: Reaction removal by target id; database dialect and message-state coverage
...
### Recent Releases
- **v0.11.0-beta.1**: Multi-DB hardening, durable reactions and read receipts, doctor DB diagnostics, WebSocket write serialization, admin TUI fixes
