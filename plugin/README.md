...
When the registry entry includes `checksum`, the server validates the SHA-256 hash of the downloaded bytes **before** extracting the archive. HTTP downloads are capped at 100 MB; oversize or checksum failures abort without writing plugin files. Archive type (`.zip`, `.tar.gz`, `.tgz`, or raw binary) is determined from the URL path, so URLs with query strings still work (for example `.../plugin.zip?token=...`). After extraction, the file whose archive entry basename matches the plugin name is marked executable (ZIP/TAR entry modes are not preserved). Local `file://` download URLs and local registry URLs are supported for development; on Unix use the three-slash form (`file:///path/to/plugin.zip` or `file:///path/to/registry.json`); on Windows both `file:///C:/path/...` and `file://C:/path/...` work. Registry and download URLs share the same parser in `plugin/fileurl`.
