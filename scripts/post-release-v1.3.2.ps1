# Post-release packaging for v1.3.2 (run after GitHub release assets are uploaded).
# Does NOT commit or push - prints commands at the end.
#
# Usage:
#   cd marchat
#   .\scripts\post-release-v1.3.2.ps1
...
Write-Host @"
cd "$RepoRoot"
git add packaging/ release-notes-v1.3.2.md
git commit -m "chore(packaging): sync v1.3.2 release checksums"
...
Write-Host ""
Write-Host '=== USER: AUR manual fallback ===' -ForegroundColor Yellow
Write-Host @"
cd C:\Users\Cody\Projects\GitHub\Personal\Active\packaging-forks
.\push-marchat-to-aur.ps1
cd aur-marchat-bin
